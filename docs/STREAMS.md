...
naming the streams this column says it produces, plus the dead-letter queues it
writes, and holds neither command on its root permission set - where a Redis key
pattern applies it to every stream the service can name, including the ones it
only reads. `tests/deploy/test_redis_acl.py` parses this column and asks redis
to prove both halves, so a producer changed here without the grant following
